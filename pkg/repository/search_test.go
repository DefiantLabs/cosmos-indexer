...
func (suite *SearchRepositorySuite) TestCreate() {
	...
	err = suite.repository.AddHash(ctx, "Qd74FlZyrasFfT97l3KLoEiKAu4bb7zPwQ30N+ZrYbI=", "block")
	suite.Assert().NoError(err)

	err = suite.repository.AddHash(ctx, "4581DF321FB0CABF43E876032061058F497C4B5F81FDACC1F851768C35294E7B+ZrYbI=", "transaction")
	suite.Assert().NoError(err)
	...
	err = suite.repository.AddHash(ctx, "4581DF321FB0CQd74FlZyrABF43E876032061058F497C4B5F81FDA", "transaction")
	suite.Assert().NoError(err)
	result, err = suite.repository.HashByText(ctx, "qd74fl")
